...
(function ($) {
	...
	function initCatalogRefresh() {
		var btn = document.getElementById('bqw-catalog-refresh');
		if (!btn) return;
		var result = document.getElementById('bqw-catalog-refresh-result');
		btn.addEventListener('click', function () {
			result.textContent = (window.BQW_Admin && window.BQW_Admin.i18n.testing) || '…';
			result.style.color = '';
			var fd = new FormData();
			fd.append('action', 'bqw_refresh_catalog');
			...
			fetch(window.BQW_Admin.ajaxUrl, { method: 'POST', credentials: 'same-origin', body: fd })
				...
				.then(function (json) {
					if (json && json.success) {
						result.textContent = '✓ ' + json.data.count + ' products';
