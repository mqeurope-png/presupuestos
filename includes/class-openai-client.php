...
final class OpenAI_Client {
	...
	public function recommend( array $client_answers, array $products ) {
		...
		$system_prompt = 'You are an expert sales advisor for Bomedia SL, a European distributor of UV-LED industrial printers, DTG/DTF textile printers, and laser cutting/engraving machines (artisJet, MBO, Flux, smartJet, pimpam brands).' . "\n\n"
			...
			. '}' . "\n\n"
			. 'Rules:' . "\n"
			. '- Return EXACTLY 6 products when at least 6 candidates exist in the catalog. If fewer than 6 candidates exist, return ALL of them. Never return fewer than 6 when 6 are available.' . "\n"
			. '- Order by score descending' . "\n"
			. '- Score is your confidence (0-100). Use it to rank, not to filter — include all 6 even if some score below 50, those simply rank lowest.' . "\n"
			. '- If absolutely no product is a reasonable fit, return an empty recommendations array and a polite fallback_message in the customer\'s language explaining their case is specific.' . "\n"
